...
1. **จุดปลาย portal (`.assistant-dock__body`) ต้องอยู่ใน DOM เสมอแม้ตอนย่อ** — ซ่อนด้วย
   `hidden`/CSS เท่านั้น ถอด element ออกเมื่อไหร่ state ของเนื้อหาผู้ช่วย (เช่นคำสั่งที่พิมพ์ค้าง)
   หายเกลี้ยง มีด่านตรวจกันไว้แล้ว
2. **แผงตรึงเต็มความสูงต้องวัดความสูงแถบนำทางจริง** (`--dock-top`) — เมนูจอแคบห่อหลายแถวได้
   ค่าคงที่เดาผิดเสมอ (กับดักเดียวกับ gl-floatbar)
3. วันที่ PRICEMETR ได้ผู้ช่วย (IP-188): เม็ดยามุมขวาล่างจะทับ `gl-basket` — ต้องเผื่อ offset ตอนนั้น
