...
if (failed === 0) {
  ...
  console.log(`สำเนาไม่ตรงกับผืนที่เผยแพร่ ${failed} ชุด — ดึงผืนฉบับใหม่มาก่อนวัดอะไรต่อ`);
  console.log("ทุกเลขที่ compare.mjs รายงาน จะเป็นเลขของผืนฉบับเก่าจนกว่าจะแก้");
}
