...
describe("what a catalogue card is allowed to say", () => {
  ...
  it("refuses to speak even when a row carries an access model, if it was never announced", () => {
    ...
    const says = describeCardClaims({ announced: false, access: "member_free", open: true, announcedAt });
  ...
  it("states access and readiness, and offers entry, once an app is announced and open", () => {
    const says = describeCardClaims({ announced: true, access: "paid_trial", open: true, announcedAt });
  ...
  it("shows the announcement date only while an announced app is still being prepared", () => {
    const preparing = describeCardClaims({ announced: true, access: "paid_trial", open: false, announcedAt });
    const open = describeCardClaims({ announced: true, access: "paid_trial", open: true, announcedAt });
  ...
  it("does not invent a date for an announced app whose row has none", () => {
    const says = describeCardClaims({ announced: true, access: "member_free", open: false, announcedAt: null });
