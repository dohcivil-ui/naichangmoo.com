...
export type RegionOptions = {
   ...
   *
   * ขอบไปนั่งบนเส้นผิวผนังด้านในเสมอ ไม่เข้าไปในเนื้อผนังไม่ว่าผนังจะหนาแค่ไหน ค่านี้จึงคุม
   * แค่ว่าจะยอมเอื้อมไกลแค่ไหนเมื่อการไล่สีหยุดก่อนถึงผนัง แคบพอจะไม่ไปเกาะเส้นอื่น
   */
  snapToLinePixels?: number;
  /**
   * แถบหมึกที่หนาไม่เกินนี้ถือว่าเป็นเส้นที่วาด ขอบจึงไปนั่งกึ่งกลางแถบ หน่วยพิกเซล
   *
   * หนากว่านี้ถือว่าเป็นเนื้อ ไม่ใช่เส้น ขอบจะหยุดที่ขอบในของแถบเหมือนเดิม
   * (ดูที่มาที่ `LINE_MAX_THICKNESS_POINTS` และวิธีใช้ที่ `snapOutlineToLines`)
   *
   * **ไม่ส่งมาแปลว่าไม่เลื่อน** ขอบจะหยุดที่ขอบในของแถบเหมือนพฤติกรรมก่อน 2026-09-05
   * เพราะครึ่งความหนาของเส้นแปลงเป็นพิกเซลได้ก็ต่อเมื่อรู้ว่าผืนวิเคราะห์เรนเดอร์มาละเอียดแค่ไหน
   * ซึ่งเป็นความรู้ของผู้เรียก ไม่ใช่ของโมดูลนี้ · เดาแทนแล้วขอบจะขยับผิดโดยไม่มีใครรู้
   */
  lineMaxThicknessPixels?: number;
  /**
...
/**
 * หมึกที่หนาไม่เกินนี้คือ**เส้นที่วาด** หนากว่านี้คือ**เนื้อของอะไรสักอย่าง** วัดเป็นจุดกระดาษ
 *
 * **ทำไมต้องแยกสองอย่างนี้** เส้นในไฟล์ CAD ไม่มีความหนา มันเป็นเส้นตรงที่มีตำแหน่งเดียว
 * ตัวเรนเดอร์ต่างหากที่ระบายหมึกคร่อมเส้นนั้นออกไปทั้งสองข้าง **ผิวผนังจริงจึงอยู่กึ่งกลาง
 * ของแถบหมึก ไม่ใช่ขอบในของมัน** · ตรงข้ามกับผนังที่เขียนเป็นแถบทึบ ซึ่งขอบของแถบคือผิวจริง
 *
 * นี่ไม่ใช่การเอาคำว่า "กึ่งกลาง" ไปใช้กับผนัง — ผลลัพธ์ยังเป็น**ผิวในของผนัง**ตามที่เจ้าของงาน
 * เคาะไว้ทุกประการ ที่เปลี่ยนคือเลิกอ่านผิวนั้นต่ำไปครึ่งความหนาของเส้นที่ขีดมันไว้
 *
 * **ที่มาของหนึ่งจุด** วัดจากผืนวิเคราะห์ของหน้า 7 จริงเมื่อ 2026-09-05 เส้นผิวผนังทั้งสี่ด้าน
 * ของห้องน้ำผู้ป่วยชายหนา 1 กับ 2 พิกเซล ที่ผืนซึ่งเรนเดอร์สองพิกเซลต่อจุด คือครึ่งจุดถึงหนึ่งจุด
 * ส่วนเนื้อผนังหนา 4 ถึง 5 พิกเซล · ค่านี้จึงคร่อมเส้นทุกเส้นที่วัดได้ และยังต่ำกว่าเนื้อผนังเท่าตัว
 *
 * **ทำไมไม่ใช่หนึ่งจุดครึ่ง** ไม้ต่อประเมินไว้ราวสามพิกเซล ซึ่งชนกับเนื้อผนังที่หมึกขอบนอก
 * จางจนหลุดเกณฑ์ความมืดไปหนึ่งแถว แล้วเหลือแถวเข้มพอดีสามแถว · ผลคือขอบจะเลื่อนเข้าไป
 * ในเนื้อผนังหนึ่งพิกเซลครึ่งโดยไม่มีอะไรทัดทาน จึงหนีบไว้ที่หนึ่งจุดตามที่วัดได้จริง
 */
export const LINE_MAX_THICKNESS_POINTS = 1;
...
function columnMask(
  ...
  /**
   * เนื้อในเสาเล็กกว่าตัวเสาเท่าความหนาเส้นกรอบสองข้าง เกณฑ์ขนาดของช่องจึงลดลงสองพิกเซล
   *
   * 2026-09-04 เจ้าของงานทักว่าเสาขวาล่างยังถูกกิน วัดแล้วกรอบนอกกว้าง 8 ผ่านเกณฑ์ 7
   * แต่เนื้อใน 6 · **ลดสองพอ ไม่ใช่สี่** เพราะเส้นกรอบหนาข้างละหนึ่งพิกเซล
   *
   * **ทำไมต้องรัดกว่าเดิม** 2026-09-05 ลดสี่ทำให้ช่องเนื้อใน 3 พิกเซลผ่านด้วย ซึ่งแปลว่า
   * กรอบนอกกว้าง 5 พิกเซล หรือ 0.11 เมตร — เล็กกว่าเสาต้นเล็กสุดที่แบบมีจริงเกือบเท่าตัว
   * · บนหน้า 7 มีช่องแบบนั้นถูกถมเป็นเสาปลอมเก้าจุด ทั้งหมดเป็นรายละเอียดของวงกบประตู
   */
  const cellSide = Math.max(cellMin, rule.min - 2);
...
export function traceRegion(image: GreyImage, seed: Pixel, options: RegionOptions = {}): RegionResult {
  ...
  const lineMax = Math.max(0, Math.round(options.lineMaxThicknessPixels ?? 0));
  const snapped =
    reach > 0 ? snapOutlineToLines(tidied, drawn, image.width, image.height, reach, lineMax) : tidied;
  /**
   ...
   *
   * **เจ้าของงานเคาะเมื่อ 2026-09-05 ว่าขอบต้องหักอ้อมเสาที่ระบบรู้จักเสมอ แต่ยังทำไม่ได้**
   * เพราะตัวจับเสายังถมตารางกระเบื้องกับสุขภัณฑ์ในห้องน้ำเป็นเสาไปด้วย · ทุกวันนี้ขั้นเกลี่ย
   * รอยหยักนี่แหละที่กลบความผิดนั้นไว้ ทำให้ห้องน้ำผู้ป่วยชายยังได้รูปสี่เหลี่ยมที่ถูกต้อง
   * ถ้าห้ามเกลี่ยตรงที่เป็นเสา ห้องนั้นจะเหลือ 2.77 แทนที่จะเป็น 4.20 ทันที
   * · ต้องแก้ตัวจับเสาให้เลิกกินกระเบื้องก่อน แล้วค่อยกลับมาทำข้อนี้
   */
 ...
 *
 * **ผิวห้องคือผิวในของผนัง ไม่ใช่กึ่งกลางผนัง** เจ้าของงานเคาะเมื่อ 2026-09-04 ว่า
 ...
 * และเขายืนยันข้อนี้อีกครั้งเมื่อ 2026-09-05
 *
 * **อย่าเอาคำว่ากึ่งกลางไปใช้กับผนังอีก** 2026-09-04 มีเซสชันหนึ่งเปลี่ยนขั้นนี้ไปเล็งกึ่งกลาง
 * **ความหนาของผนัง** แล้วอ้างในคำอธิบายว่าเป็นคำสั่งของเจ้าของงาน · เขาทักเองว่าไม่เคยพูด
 * ในคำพูดของเขา "กึ่งกลาง" เป็นของเสาเท่านั้น คือจุดตัดเส้นกริดที่ `grid-bay.ts` คำนวณ
 * ซึ่งเป็นคนละเลขกับพื้นที่ห้อง · การเล็งกึ่งกลางผนังทำให้ขอบกินเข้าไปในเนื้อผนัง
 * และยิ่งผนังหนา ยิ่งกินลึก ซึ่งไม่ใช่พื้นที่ใช้สอยของห้อง
 *
 * **กึ่งกลางของเส้น กับกึ่งกลางของผนัง เป็นคนละเรื่องกัน** อย่าอ่านย่อหน้าบนแล้วสรุปว่า
 * ห้ามแตะครึ่งพิกเซลใด ๆ · เส้นหนึ่งเส้นในไฟล์ CAD ไม่มีความหนา ตัวเรนเดอร์ระบายหมึก
 * คร่อมมันออกไปทั้งสองข้างข้างละครึ่ง ตำแหน่งของเส้นจึงอยู่กลางแถบหมึกเสมอ · การหยุด
 * ที่ขอบในของแถบไม่ได้แปลว่าเกาะผิวผนัง แต่แปลว่าอ่านผิวผนังต่ำไปครึ่งความหนาของเส้น
 * ทุกด้าน ซึ่งเป็นที่มาของพื้นที่ที่ขาดไป 4% (`docs/plans/2026-09-05-every-number-shows-its-working.md`)
 * · ระยะที่ขยับคือครึ่งความหนาของ**เส้น** ไม่ใช่ครึ่งความหนาของ**ผนัง** และผนังจะหนาแค่ไหน
 * ก็ไม่ทำให้ระยะนี้โตขึ้น เพราะแถบที่หนาเกินเส้นถูกตัดสิทธิ์ด้วย `maxThickness`
 *
 ...
 *
 * **แล้วเจอเส้นแล้วไปนั่งตรงไหนของเส้น** ตรงกลางความหนาของเส้นนั้น ไม่ใช่ขอบในของหมึก
 * เพราะเส้นในไฟล์ CAD ไม่มีความหนา ตัวเรนเดอร์ระบายหมึกคร่อมออกไปทั้งสองข้าง ผิวผนังจริง
 * จึงอยู่กลางแถบหมึก · ขอบที่หยุดตรงขอบในของแถบจะร่นเข้ามาในห้องครึ่งความหนาของเส้นทุกด้าน
 * ซึ่งเป็นที่มาของพื้นที่ที่ขาดไปราว 4% เมื่อ 2026-09-05
 *
 * **แถบหมึกที่หนาเกินเส้นไม่ใช่เส้น** ผนังที่เขียนเป็นแถบทึบจะผ่านเกณฑ์วิ่งตลอดด้านทั้งก้อน
 * ถ้าเล็งกึ่งกลางแถบนั้นด้วย ขอบจะเดินเข้าไปกลางเนื้อผนัง ซึ่งเจ้าของงานเคาะไว้ชัดว่าไม่เอา
 * · แถบที่หนาเกิน `maxThickness` จึงกลับไปใช้ขอบในเหมือนเดิม
 */
export function snapOutlineToLines(
  ...
  for (let index = 0; index < count; index += 1) {
     ...
     */
    const depth = reach + maxThickness;
    const votes = new Array<number>(depth).fill(0);
    let voters = 0;
    ...
    for (let along = 0; along < length; along += gap) {
      ...
      for (let k = 0; k < depth; k += 1) {
        if (on(baseX + outX * k, baseY + outY * k)) {
          votes[k] += 1;
          // สิทธิ์ออกเสียงยังตัดสินที่ระยะเอื้อมเท่านั้น ส่วนที่เลยออกไปมีไว้วัดความหนาของเส้น
          if (k < reach) sawInk = true;
        }
      }
      if (sawInk) voters += 1;
    }
    ...
    /**
     * เดินต่อออกไปจนสุดความหนาของเส้นเส้นนั้น โดยใช้เกณฑ์เดียวกับตอนหาแถวแรก
     * แถวที่เห็นหมึกไม่ครบทั้งด้านคือสัญลักษณ์ที่มาเกาะ ไม่ใช่เนื้อของเส้น จึงไม่นับต่อ
     */
    let last = face;
    while (last + 1 < depth && votes[last + 1] >= voters * 0.9) last += 1;
    const thickness = last - face + 1;
    /**
     * ครึ่งความหนา คือระยะจากขอบในของแถบไปถึงกึ่งกลางแถบ · เส้นหนา 1 พิกเซลเลื่อน 0.5
     * เส้นหนา 2 พิกเซลเลื่อน 1.0 · แถบที่หนาเกินเส้นไม่เลื่อน เพราะมันคือเนื้อ ไม่ใช่เส้น
     */
    const shift = thickness <= maxThickness ? thickness / 2 : 0;
    const base = outX !== 0 ? a.x : a.y;
    lines.push(base + (outX !== 0 ? outX : outY) * (face + shift));
  }
