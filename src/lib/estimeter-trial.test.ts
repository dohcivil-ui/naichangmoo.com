...
describe("ESTIMETR trial window", () => {
  // ADR 0009 sets seven days. The date is written out rather than derived from the constant so
  // that changing the constant fails here, which is the point at which a superseding ADR is due.
  it("runs exactly seven days from the moment of activation", () => {
    ...
    expect(window.endsAt.toISOString()).toBe("2026-08-30T09:15:00.000Z");
  });
  ...
  it("counts a partial day as a remaining day and stops at zero", () => {
    const endsAt = new Date("2026-08-30T09:15:00.000Z");

    expect(trialDaysRemaining(endsAt, new Date("2026-08-23T09:15:00.000Z"))).toBe(7);
    expect(trialDaysRemaining(endsAt, new Date("2026-08-29T21:15:00.000Z"))).toBe(1);
    expect(trialDaysRemaining(endsAt, new Date("2026-08-30T09:15:00.000Z"))).toBe(0);
  ...
  it("produces an entitlement that matches the approved trial policy end to end", () => {
    ...
    const afterExpiry = new Date("2026-08-31T00:00:00.000Z");
