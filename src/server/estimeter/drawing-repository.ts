...
/** แบบหนึ่งไฟล์ พร้อมสเกลทุกหน้าและรอยวัดทุกหน้าของมัน — วัตถุดิบของ backup sheet */
export type BackupSheetSource = {
  documentId: string;
  /**
   * ลายนิ้วมือของไฟล์ที่เบราว์เซอร์คิดตอนเปิด · ไม่ใช่ชื่อไฟล์
   *
   * backup sheet ต้องบอกได้ว่าตัวเลขชุดนี้อ่านมาจากไฟล์ไหน แต่ระบบไม่เก็บชื่อไฟล์เลย
...
/**
 * อ่านทุกอย่างที่ backup sheet ต้องใช้ ของทั้งโครงการในครั้งเดียว (IP-243)
 *
 * **ต่างจาก `loadDrawingState` ตรงที่ไม่ต้องรู้ว่าเป็นไฟล์ไหน** ตัวนั้นเริ่มจาก `documentId`
 * ซึ่งเบราว์เซอร์ได้มาจากการคิดลายนิ้วมือของไฟล์ที่คนเพิ่งเปิด · backup sheet เปิดจากที่ไหนก็ได้
 ...
 * เปิดแบบได้หลายไฟล์ (สถาปัตย์ โครงสร้าง งานระบบ) และ backup sheet ของโครงการต้องครบทั้งโครงการ
 *
 * **ไม่แตะสถานะมุมมองของผู้ใช้** เพราะมุมมองคือเรื่องของคนที่กำลังทำงาน ไม่ใช่ของที่ยื่นให้คนตรวจ
 ...
 */
export async function loadBackupSheetSource(
  organizationId: string,
  projectId: string
): Promise<BackupSheetSource[]> {
  const db = getDb();
  const documents = await db
    .select({
