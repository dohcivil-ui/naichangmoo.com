...
import { auditEvents, evidenceReferences, projects, takeoffItems, takeoffRuns } from "@/db/schema";
...
export async function confirmManualItem(input: {
  ...
  return getDb().transaction(async (tx) => {
    ...
    const evidenceCount = counted[0]?.value ?? 0;

    if (itemConfirmationBlocker({ reviewState: current.reviewState, evidenceCount })) {
