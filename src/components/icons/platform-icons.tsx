...
/**
 * ลูกศรขวา — **รูปทรงเดียวกับลูกศรของปุ่ม** ที่ `button.tsx` วาดไว้แล้ว ไม่ใช่ทรงที่สอง
 *
 * ต้นฉบับอยู่ที่ `<Arrow>` ใน `button.tsx` บนกริด 20 · `M3.5 10h12` กับ
 * `M11 5.5l4.5 4.5-4.5 4.5` · ที่นี่กริด 48 จึงคูณทุกพิกัดด้วย 2.4 ตรง ๆ ไม่ปัดเศษ
 * ปลายเส้นนอนกับปลายหัวลูกศรจึงยังบรรจบที่ x เดียวกันคือ 37.2 เหมือนต้นฉบับที่ 15.5
 *
 * เว้นว่างไว้แต่แรกเพราะรอภาพจริง แล้วกลายเป็นว่าภาพจริงมีอยู่แล้วในบ้านตัวเอง ·
 * เจ้าของงานตัดสิน 2026-09-06 ว่าห้ามให้หน้าเดียวมีลูกศรสองทรง
 *
 * **ลูกศรซ้ายไม่วาดตัวที่สอง** ให้พลิกตัวนี้ด้วย CSS `transform: scaleX(-1)` เอา
 *
 * เส้นบางกว่าต้นฉบับตามสัดส่วน ต้นฉบับหนา 1.9 บนกริด 20 (0.095) ตัวนี้รับ 2.4 บนกริด 48
 * (0.05) จาก `BaseIcon` ตามความหนามาตรฐานของไอคอนทั้งเว็บ · อยากได้หนาเท่าตาเห็น
 * ส่ง `strokeWidth={4.56}` ตอนเรียก ซึ่งคือ 0.095 คูณ 48
 */
export function ArrowRightIcon(props: IconProps) {
  return <BaseIcon {...props}><path d="M8.4 24h28.8" /><path d="M26.4 13.2l10.8 10.8-10.8 10.8" /></BaseIcon>;
}
