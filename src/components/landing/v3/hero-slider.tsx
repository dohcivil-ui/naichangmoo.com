...
export function HeroSlider({ slides }: { slides: readonly HeroSlideView[] }) {
  ...
  useEffect(() => {
     ...
     *
     * **นี่คือทางหยุดทางเดียวที่เหลืออยู่ — เจ้าของงานเคาะ 2026-09-07 ให้ยึดผืน**
     * เดิมสไลด์หยุดเมื่อชี้เมาส์และเมื่อโฟกัสเข้าแบนเนอร์ด้วย ซึ่งเป็นทางที่ WCAG 2.2.2
     * ยอมรับสำหรับของที่ขยับเองนานเกินห้าวินาที · ผืนเป็นภาพนิ่งจึงไม่มีเรื่องนี้ให้เห็น
     * และคำสั่งยึดผืนถอนทั้งสองทางออก · **สิ่งที่เสียไปคือคนที่อ่านช้าจะถูกสไลด์เลื่อนหนี
     * กลางประโยค และคนที่ใช้คีย์บอร์ดกดลูกศรเลือกสไลด์แล้วจะถูกหมุนแย่งกลับในห้าวินาที**
     * ทางนี้เก็บไว้เพราะมันไม่ใช่สิ่งที่ผืนวาด มันคือคำสั่งที่ผู้ใช้ตั้งไว้ที่เครื่องของเขาเอง
     */
     ...
     * มีกี่ใบ และไม่มีทางหยุดเลยสักทาง เพราะการหยุดตอนชี้กับตอนโฟกัสถูกถอนไปแล้ว
     * นั่นแย่กว่าที่ผืนวาด ไม่ใช่ตามผืน · คำว่าการ์ดนิ่งแปลว่าไม่ขยับ
     *
     * **จุดตัด 760 อยู่สองที่** ที่นี่กับบล็อกมือถือใน `globals.css` · ย้ายที่ไหน
     * ต้องย้ายทั้งสองที่พร้อมกัน ไม่งั้นจะมีช่วงความกว้างที่สไลด์หมุนโดยไม่มีปุ่มให้กด
     */
    ...
    /* **อ่านค่าครั้งเดียว ไม่ได้ฟังการเปลี่ยน** คนที่เปิดหน้าโดยตั้งลดการเคลื่อนไหวไว้
       ...
       คือคนที่เปิดการลดการเคลื่อนไหวระหว่างดู จะไม่โดนสไลด์หมุนต่อ เพราะการเปลี่ยนสไลด์
       ทำให้ effect นี้ตั้งใหม่ทุกห้าวินาทีอยู่แล้ว จึงอ่านค่าใหม่รอบถัดไปเสมอ · ยังไม่ผูก
       listener เพราะเป็นทางที่ไม่เร่ง และ listener ที่ไม่มีใครทดสอบมีราคาสูงกว่าที่มันแก้ */
  ...
  return (
    ...
    >
      ...
      {/* คนที่ใช้โปรแกรมอ่านหน้าจอไม่ได้เห็นภาพเปลี่ยน จึงต้องบอกว่าบริเวณนี้เปลี่ยนเองได้
          ...
          อ่านทั้งสามใบใช้ปุ่มเลื่อนได้ ซึ่งเป็นปุ่มจริงที่แท็บถึง · **แต่การกดมันไม่หยุด
          การหมุนอีกแล้ว** ตั้งแต่คำสั่งยึดผืน 2026-09-07 อีกห้าวินาทีสไลด์จะเลื่อนต่อเอง */}
