...
4. Give the work a roadmap item id before implementation and use it in every commit message from the first one. The roadmap *file* is written when the version is cut, not when the work starts: the pointer must stay byte-identical to its versioned file, so editing it mid-stream would force a version bump for every commit, which is exactly what rule 5 removed.
5. **A commit and a version are different things, and the rules for them are different.** Commit whenever one coherent piece of work is finished and the quality gate is green. A version is cut only when the work is deployable. This split was made on 2026-08-30 after 96 versions in 10 days: forcing a version bump per commit did not make rollback easier, it made commits larger, because a change that had to be one version had to be one commit. Rollback gets easier with small commits, not with few of them.
...
7. Run the quality gate required by the changed code before committing. This is the whole bar for a commit: green `lint`, `typecheck` and `test`, one coherent change, and a message that says why. No roadmap version, no CHANGELOG entry and no handoff note is required to commit.
8. **Cutting a version** means all of it, in one commit at the end of the work: the versioned roadmap file, the `roadmap.json` pointer that must be a byte-identical copy of it, the `package.json` version, the CHANGELOG entry, and a dated handoff note stating changed files, verification, risk, rollback and next action. A version may contain several commits, and normally does. What makes something a version is that it can be deployed and rolled back as a unit — the migration is applied and checked, the real path has been walked in the browser, and the handoff explains what changed.
9. **Hunt for bugs in the code you touch, and report them.** Delivering only what was asked is half the job: the owner is a civil engineer, not a programmer, so a session that reads a file and says nothing has spent the one chance anybody had to notice what is wrong in it. Report each find with the failure it causes, not "this looks odd". Fix it in the same commit only when it sits inside the change already being made; otherwise name it and let him choose. This rule was set on 2026-09-05 after two finds in one session — the assistant dock's floating pill wrapping to three lines because `.assistant-dock` has no width, and a page scale that could be set but never cleared.
10. Close every version with `pnpm release`, which tags, pushes and publishes the GitHub Release together. A GitHub Release is not the same object as a tag; publishing only the tag is what left the repository showing a release twenty-five versions old. Because a version is now rarer than a commit, this step carries the strictness that rule 5 used to spread across every commit: never tag work that has no handoff note and no versioned roadmap file.
